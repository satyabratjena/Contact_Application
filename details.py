...
def search():
    if request.method == "POST":
        ...
        if cont_cap.isalpha(): 
            contact_ifalpha= Contact.query.filter(Contact.name.like('%'+cont_cap+'%')).all()
            return render_template("home.html",contact=contact_ifalpha)
        elif cont_cap.isdigit():
            cont_ph = Contact.query.filter(Contact.number.like('%'+cont_cap+'%')).all()
...
def c_update():
    if request.method == 'POST': #getting this post the form
...
@app.route("/delete/<srno>/", methods = ['GET','POST']) #method is not required
